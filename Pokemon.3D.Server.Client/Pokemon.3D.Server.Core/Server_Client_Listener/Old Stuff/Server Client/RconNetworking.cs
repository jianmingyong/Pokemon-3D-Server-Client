...
namespace Pokemon_3D_Server_Core.Server_Client_Listener.Network
{
    /// <summary>
    ...
    public class RconNetworking : IDisposable
    {
            ...
            do
            {
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(ReturnMessage))
                    {
                        ...
                        Core.Logger.Log("RconNetworking.cs: Receive: " + ReturnMessage, Logger.LogTypes.Debug, Client);
        ...
        public void StartSending(Package p)
        {
            try
            {
                ...
                Writer.Flush();
                Core.Logger.Log("RconNetworking.cs: Sent: " + p.ToString(), Logger.LogTypes.Debug, Client);
            }
            catch (Exception)
            {
                return;
